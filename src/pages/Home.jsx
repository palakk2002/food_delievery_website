 ...
 let total=Math.floor(subtotal+deliveryFee+taxes);
    ...
    return (
         <div className=' bg-slate-200 w-full min-h-screen'>
          ...
          <div className="text-center text-2xl text-green-500 font-semibold p-t-5">No matches found ...</div>}
           ...
           <RxCross2 className="w-[30px] h-[30px] text-green-400 text-[18px] font-semibold cursor-pointer hover:text-gray-600" onClick={()=>setShowCart(false)} />
            ...
            {items.length>0?<>
                   <div className="wifull mt-9 flex flex-col gap-8 ">
        {items.map((item)=>(
...
<div className="w-full flex justify-between items-center">
  ...
  <span className="text-lg text-green-400 font-semibold">Rs{taxes}/-</span></div>
  ...
  <span className="text-2xl text-green-400 font-semibold">Rs{total}/-</span></div>
<button className="w-[80%] p-3 rounded-lg bg-green-500 text-white hover:bg-green-400 transition-all" onClick={()=>{
  toast.success("Order Placed..")
}}>Place Order</button>
</>:
<div className="text-center text-2xl text-green-500 font-semibold p-t-5">Empty Cart..</div>}
